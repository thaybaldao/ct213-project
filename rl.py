...
class RLAlgorithm:
    def __init__(self, train_timesteps, simulation_timesteps, n_eval_episodes, net_size, learning_rate_size, gamma_size, actor_size, critic_size, env_name):
                ...
                for gamma_idx in range(self.gamma_size):
                    for actor_idx in range(self.actor_size):
                        for critic_idx in range(self.critic_size):
                            ...
                            dictionary, model_name = self.train(learning_idx, gamma_idx, actor_idx, critic_idx, net_layers, hyp_id)
    ...
    def train(self, learning_idx, gamma_idx, actor_idx, critic_idx, net_layers, hyp_id):
        ...
        learning_rate, gamma, policy_kwargs, policy_info = self.choose_params(learning_idx, gamma_idx, actor_idx, critic_idx, net_layers)
        # Create the model
    ...
    def save_json(self, model_name, dictionary):
        models_dir_name = self.log_dir_name + "models/"
        models_dir = os.getcwd() + "/" + models_dir_name
        os.makedirs(models_dir, exist_ok=True)
        with open(models_dir_name + model_name + ".json", "w") as outfile:
